    ...
    <section class="pago-section">
        <div class="container" style="max-width:400px;margin:2rem auto;background:#fff;padding:2rem;border-radius:1rem;box-shadow:0 2px 16px #0002;">
            ...
            <?php if (isset($ordenId) && isset($total)): ?>
                <a href="/pago/descargarComprobante/<?= $ordenId ?>/<?= $total ?>" class="btn-pagar" id="descargar-comprobante" style="display:block;text-align:center;margin-top:2rem;">Descargar comprobante PDF</a>
            <?php endif; ?>
    ...
    <script>
        <?php if (isset($ordenId) && isset($total)): ?>
            document.addEventListener('DOMContentLoaded', function() {
                var btn = document.getElementById('descargar-comprobante');
                if (btn) {
                    btn.addEventListener('click', function(e) {
                        // Permite la descarga normal
                        setTimeout(function() {
